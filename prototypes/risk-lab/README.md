...
阅读预览已支持 Wiki/普通库内链接、标题链接及折叠的 Front Matter 属性。库内链接在应用内打开；失效路径、失效标题和不支持的附件明确提示。媒体嵌入暂为原路径占位，不会自动加载外部资源。属性错误不改原文。

独占段落的 `![[raw/Areas/主题.md]]` 展开正文，`![[raw/Areas/主题.md#标题|别名]]` 只展开该标题及下级小节。嵌入中的相对链接以来源文档为准；来源按钮可打开原文。句子中间的嵌入只显示“打开原文”链接。循环、失效标题或超限会提示，不改原文；单次预览最多嵌套 4 层、展开 24 次、累计解析 20 万字符的来源正文（标题引用也计整篇解析量）。不支持块 ID、外部正文或媒体自动加载。
