...
完整状态汇总、待确认问题和下一步范围讨论入口见[需求总览与查漏补缺](./overview.md)。以下列表是专题索引，不代表首版全量承诺。
...
- 资料导入与批量迁移暂不实现，待出现实际大量导入需求时再评估；不影响开放格式及外部文件编辑支持。
- 既有的版本记录、修改审计、必要系统通知和每日记忆整理不受影响。

这里只记录进阶想法；完整分层图谱、原生客户端与安卓控件、系统推送属于已确认的后续实施需求，不能与可选想法混淆。完整版本边界以 [Web 首版](./web-first-release.md)为准。
