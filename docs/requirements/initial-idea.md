...
- 第一期不实现用户主动设置的普通事项提醒、文档回顾提醒和周期提醒。
- 第一期不实现基于记忆曲线的间隔复习，包括复习队列、掌握程度反馈和 AI 复习提问。
- 以上功能保留为后续进阶方向，具体交互、算法和实施时间尚未确定；此前讨论的方案仅作为候选，不视为已确认需求。
- 本次延期不影响已经确认的系统通知，例如任务完成、同步或整理失败、待确认事项，以及相应的客户端提醒和手机推送。
- 每日凌晨 3 点的记忆整理属于后台知识加工，不是间隔复习，不因本次延期取消。
